...
Список существующих контейнеров (запущенных и остановленных) осущаствляется с помощью команды `docker ps`
````bash
docker ps -a
````
Основные опции
...
Запуск сущетсвующего контенера
````bash
docker start [name | id]
````
Остановка сущетсвующего контенера
````bash
docker stop [name | id]
````
Перзапуск сущетсвующего контенера
````bash
